...
export default function Mint(props: Props) {
  ...
  async function mint(event:React.FormEvent) {
    ...
    contract.mint(collection, toAddress, parseUnits(tokenId, 18), tokenUri)
  ...
  return (
    ...
    <FormControl>
      ...
      <Input id="to-address" type="text" required  onChange={(e) => setToAddress(e.target.value)} my={3}/>

      <FormLabel htmlFor='token_id'>Token ID: </FormLabel>
      <NumberInput defaultValue={tokenId} min={0} max={1000000} onChange={handleChangeTokenId}>
        <NumberInputField />
      </NumberInput>

      <FormLabel htmlFor='token_uri'>Token URI: </FormLabel>
      <Input id="token-uri" type="text" required  onChange={(e) => setTokenUri(e.target.value)} my={3}/>

      <Button type="submit" isDisabled={!currentAccount}>Mint a new NFT</Button>
    </FormControl>
    </form>
  )
}
