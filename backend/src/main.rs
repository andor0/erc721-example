...
use web3::{error::Error as Web3Error, futures::{future, StreamExt}, types::{FilterBuilder, H256}};

// web3.utils.keccak256('CollectionCreated(address,string,string)')
const TOPIC_COLLECTION_CREATED: H256 = H256(hex!("3454b57f2dca4f5a54e8358d096ac9d1a0d2dab98991ddb89ff9ea1746260617"));
// web3.utils.keccak256('TokenMinted(address,address,uint256,string)')
const TOPIC_TOKEN_MINTED: H256 = H256(hex!("c9fee7cd4889f66f10ff8117316524260a5242e88e25e0656dfb3f4196a21917"));

type Events = Vec<Event>;

#[derive(Serialize)]
enum Event {
    CollectionCreated { collection: String, name: String, symbol: String },
    TokenMinted {collection: String, recipient: String, token_id: String, token_uri: String },
}

impl Event {
    fn from_log(log: web3::types::Log) -> Result<Event, Web3Error> {
        match (&log.topics.contains(&TOPIC_COLLECTION_CREATED), &log.topics.contains(&TOPIC_TOKEN_MINTED)) {
            (true, false) => {
                ethabi::decode(
                    &[
                        ParamType::Address,
...
async fn event_listener(state: SharedState) -> Result<(), web3::Error> {
    ...
    let mut sub = web3.eth_subscribe().subscribe_logs(
...
async fn main() -> web3::Result {
    ...
    let _ = futures::join!(event_listener(shared_state.clone()), http_server(shared_state));
